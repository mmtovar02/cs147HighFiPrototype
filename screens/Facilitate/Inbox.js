...
export default function Inbox({ navigation, route }) {
  ...
  if (route.params && route.params.askState != askState) {
    setAskState(route.params.askState);
    route.params = false;
  }
  ...
  const getHeader = () => {
    return askState ? (
      <Header
        ...
        onRightButtonPress={() =>
          navigation.navigate("Conversation", { discussionTopic: true })
        }
      />
    ) : (
      <Header
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView>
        <DiscussionTopic
          displayAskButton={!askState}
          onAskButtonPress={() => setAskState(!askState)}
        />
